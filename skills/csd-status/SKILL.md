...
name: csd-status
description: >
  Show current CSD project state and what to do next. Use at the start of a
  new session to resume work, or anytime to check progress. Reads .csd/STATE.md
  and project artifacts to give a complete picture. Trigger: "what's next",
  "where are we", "csd status", "resume", "/csd-status".
---

# /csd-status — Where Are We, What's Next
...
Read `.csd/STATE.md`. If it doesn't exist:
- Tell the user there's no active CSD project
- Check if `.csd/BACKLOG.md` exists and mention pending backlog items
- Suggest `/csd-start` to begin a new project
- Stop here
...
- Say: "You're in research mode. Keep exploring, or run `/csd-plan` when you're ready to plan."
...
- Say: "Run `/csd-plan` to continue planning, or `/csd-execute` if the plan is ready."
...
- Say: "Run `/csd-execute` to continue, or `/csd-complete` if you're done."

**complete:**
- Say the project is complete and suggest `/csd-start` for a new one
...
This command is designed for the "fresh session" use case. When a user starts a new chat and runs `/csd-status`, they should get everything they need to continue working without reading multiple files themselves. Be thorough but concise — give them the full picture in one response.
